...
class Boards
  ...
  def last_winner
    return nil if @boards.count(&:winner?) < @boards.count
  ...
  def add_winners
    @boards.select do |board|
      next board if @winners.map(&:id).include?(board.id)

      @winners.push(board) if board.winner?
    end
  end
end
...
class Board
  ...
  def winner?
    @marks.values.map(&:first).tally.values.any? { |qtd| qtd == 5 } ||
      @marks.values.map(&:last).tally.values.any? { |qtd| qtd == 5 }
  end
...
def solution
  ...
  numbers_from_input.each do |number|
    ...
    return @boards.last_winner.score if @boards.last_winner
  end
end

class Test < Minitest::Test
  def test_solution
    assert_equal 1924, solution
  end
end
